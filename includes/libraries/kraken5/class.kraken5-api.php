...
if( !class_exists( 'TWITCHPRESS_Kraken5_API' ) ) :

class TWITCHPRESS_Kraken5_API {
    ...
    public static function administrator_main_account_listener() {
        ...
        if( !$new_token['token'] ) {        
            $bugnet->trace( 'oauth2mainaccount',
                ...
                __FILE__,
                true,
                __( 'Kraken did not return token.', 'twitchpress' )
            );      
    ...
    */
    ...
    public function generateToken( $code = null ){
        ...
        if ( is_array( $result ) && array_key_exists( 'access_token', $result ) )
        {
            ...
            $this->bugnet->log( __FUNCTION__, sprintf( __( 'Access token returned: %s', 'twitchpress' ), $token['token'] ), array(), true, false );
        } 
        else 
        {
            $token['token'] = false;
            $token['grants'] = array();
            $this->bugnet->log( __FUNCTION__, sprintf( __( 'No access token returned: %s', 'twitchpress' ), $token['token'] ), array(), true, false );
        }
