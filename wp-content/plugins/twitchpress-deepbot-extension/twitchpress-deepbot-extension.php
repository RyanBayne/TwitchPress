<?php 
/*
Plugin Name: TwitchPress DeepBot Extension
Version: 1.0.0
Plugin URI: http://twitchpress.wordpress.com
Description: DeepBot API extension for our Twitch.tv plugin.
Author: Ryan Bayne
Author URI: http://ryanbayne.wordpress.com
Text Domain: twitchpress-deepbot
 ...
 */
define( 'TWITCHPRESS_DEEPBOT_VERSION', '1.0.0' );
define( 'TWITCHPRESS_DEEPBOT_MIN_PHP_VER', '5.6.0' );
define( 'TWITCHPRESS_DEEPBOT_MIN_TP_VER', '1.6.1' );
define( 'TWITCHPRESS_DEEPBOT_MAIN_FILE', __FILE__ );
define( 'TWITCHPRESS_DEEPBOT_PLUGIN_URL', untrailingslashit( plugins_url( basename( plugin_dir_path( __FILE__ ) ), basename( __FILE__ ) ) ) );
define( 'TWITCHPRESS_DEEPBOT_PLUGIN_PATH', untrailingslashit( plugin_dir_path( __FILE__ ) ) );

if ( ! class_exists( 'TwitchPress_DeepBot' ) ) :

    class TwitchPress_DeepBot {
        /**
         * @var Singleton
         */
        ...
        private function define_constants() {
            ...
            if ( ! defined( 'TWITCHPRESS_DEEPBOT_ABSPATH' ) )  { define( 'TWITCHPRESS_DEEPBOT_ABSPATH', __FILE__ ); }
            if ( ! defined( 'TWITCHPRESS_DEEPBOT_BASENAME' ) ) { define( 'TWITCHPRESS_DEEPBOT_BASENAME', plugin_basename( __FILE__ ) ); }
            if ( ! defined( 'TWITCHPRESS_DEEPBOT_DIR_PATH' ) ) { define( 'TWITCHPRESS_DEEPBOT_DIR_PATH', plugin_dir_path( __FILE__ ) ); }
            
            // Constants for force hidden views to been seen for this plugin. Do not change the constant for each extension created.
        ...
        */
        public function update_system_scopes_status( $filtered_array ) {
            ...
            $scopes['admin']['twitchpress-deepbot-extension']['required'] = array();
            
            // Scopes for admin only or main account features that may not be used.
            $scopes['admin']['twitchpress-deepbot-extension']['optional'] = array(); 
                        
            // Scopes for functionality that is always used. 
            $scopes['public']['twitchpress-deepbot-extension']['required'] = array();
            
            // Scopes for features that may not be used.
            $scopes['public']['twitchpress-deepbot-extension']['optional'] = array(); 
        ...
        */
        public function settings_add_section_users( $sections ) {  
            ...
            $new_sections = array(
                'testsection'  => __( 'Test Section', 'twitchpress-deepbot' ),
            );
        ...
        public function settings_add_options_users( $settings ) {
            ...
            if ( 'testsection' == $current_section || !$current_section && $only_section ) {
                $new_settings = apply_filters( 'twitchpress_testsection_users_settings', array(
                    ...
                    array(
                        'title' => __( 'Testing New Settings', 'twitchpress-deepbot' ),
                        'type'     => 'title',
                    ...
                    array(
                        'desc'            => __( 'Checkbox Three', 'twitchpress-deepbot' ),
...
if( !function_exists( 'TwitchPress_DeepBot_Ext' ) ) {

    function TwitchPress_DeepBot_Ext() {        
        return TwitchPress_DeepBot::instance();
    }

    // Global for backwards compatibility.
    $GLOBALS['twitchpress-deepbot'] = TwitchPress_DeepBot_Ext(); 
}

// Activation and Deactivation hooks.
register_activation_hook( __FILE__, array( 'TwitchPress_DeepBot', 'install' ) );
register_deactivation_hook( __FILE__, array( 'TwitchPress_DeepBot', 'deactivate' ) );